...
import { modelforgeApi } from "@/lib/modelforge-api";
...
export function useModelforge() {
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      // Real backend call: ModelForge's POST /api/v1/complete does its own
      // task classification, policy-constrained routing, and execution
      // (with fallback) server-side, then reports back which provider/model
      // actually answered. The modelKey guess above only drives the local
      // UI animation (which model card lights up) while the request is in
      // flight; the response below reflects what the backend really picked.
      try {
        const result = await modelforgeApi.complete(text);

        if (result.succeeded && result.answer) {
          setMessages((prev) => [
            ...prev,
            {
              id: genId(),
              role: "assistant",
              model: result.model_id ?? models[modelKey].name,
              content: result.answer as string,
            },
          ]);
          setRouterStatus(
            `Response complete · served by ${result.provider_id}/${result.model_id} · 0 external calls`
          );
          addLog(`Routed to ${result.provider_id}/${result.model_id} (task: ${result.task_type})`, "ok");
        } else {
          const reason = result.decision_reasons?.[0] || "No eligible model for this request.";
          setMessages((prev) => [
            ...prev,
            {
              id: genId(),
              role: "assistant",
              content: `Router could not complete this request: ${reason}`,
            },
          ]);
          setRouterStatus("Request failed · no eligible/available model");
          addLog(`Routing failed: ${reason}`, "warn");
        }
      } catch (err) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: genId(),
            role: "assistant",
            content: `Could not reach the ModelForge backend (${message}). Is it running at the configured MODELFORGE_BACKEND_URL?`,
          },
        ]);
        setRouterStatus("Backend unreachable");
        addLog(`Backend request failed: ${message}`, "warn");
      }
